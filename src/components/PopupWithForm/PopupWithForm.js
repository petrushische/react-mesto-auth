...
function PopupWithForm(props) {
 return (
  <div className={`popup popup${props.name} ${props.isOpen}`}>
   ...
   <form className={`popup__container popup__container${props.name}`} name={props.name} noValidate>
    <h2 className={`popup__title popup__title${props.name}`}>{props.title}</h2>
    {props.children}
    <button className={`popup__save popup__save${props.name}`}>Сохранить</button>
   </form>
  </div>
 )
}
